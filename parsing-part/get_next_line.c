...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/11 12:10:51 by obarais           #+#    #+#             */
/*   Updated: 2025/05/11 14:52:05 by obarais          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char    *ff_strjoin(char *remains, char *buffer)
{
    char *array;
    unsigned int size;
    int i;
    ...
    if (!remains && !buffer)
        return (NULL);
    size = ff_strlen(remains) + ff_strlen(buffer);
...
char     *get_next_line(int fd)
{
    ...
    count = 1;
    while (!ff_strchr(buffer) && count != 0)
    {
        if ((count = read(fd, buffer, 1024)) == (-1))
            return (NULL);
        buffer[count] = '\0';
        remains = ff_strjoin(remains, buffer);
    }
